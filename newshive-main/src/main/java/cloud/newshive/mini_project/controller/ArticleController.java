...
public class ArticleController {
    ...
    @GetMapping("/search")
    public String searchArticles(@RequestParam("q") String query, @RequestParam(defaultValue = "1") int page,
        ...
        try {
            ResponseEntity<String> responseBody = articleService.searchArticles(query, page);
    ...
    public String getCategoryArticles(@PathVariable String category, HttpSession session, Model model) {
        ...
        try {
            ...
            logger.error("Error fetching category articles: " + e.getMessage());
        }
    ...
    public String getBookmarkedArticles(HttpSession session, Model model) {
        ...
        if (email != null && isAuthenticated) {
            ...
            return "error"; // Handle not authenticated case
        }
    }
