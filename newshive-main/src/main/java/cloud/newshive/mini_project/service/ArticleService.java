...
public class ArticleService {
    ...
    @Autowired
    @Qualifier("secretsConfigProperties")
    ...
    public ResponseEntity<String> getTopHeadlines() throws Exception {
        ...
        headers.set("x-api-key", secretsConfig.getNewsApiKey()); // Use getter here
    ...
    public List<Article> storeCategoryArticles(String body, String category) {
        ...
        JsonReader jsonReader = Json.createReader(new StringReader(body));
        JsonObject jsonObject = jsonReader.readObject();
        JsonArray articleArray = jsonObject.getJsonArray(category); // Use the category provided
    ...
    // Method to map from a Map<String, String> to an Article object
    ...
    private void addFieldIfNotNull(JsonObjectBuilder builder, String key, String value) {
        if (value != null) {
            ...
            builder.add(key, JsonValue.NULL);  // Or you can omit this line to not add the key if the value is null.
        }
    }
